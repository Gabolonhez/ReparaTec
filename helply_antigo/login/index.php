...
if (!empty($mandar)){
    ...
    if ($cpf != $db_cpf){
        ...
        // Atualiza o status para Logado do usuário no banco de dadods
        $sql_update = "UPDATE users SET logged = 1 WHERE user_cod = ?";
        $stmt_update = mysqli_prepare($conn, $sql_update);
        mysqli_stmt_bind_param($stmt_update, "s", $user_cod);
